...
exports.builder = yargs => {
  ...
  return yargs
    ...
    .check(argv => {
      ...
      // Since 9.0.0 Mocha's handleRequires is async; we currently
      // don't wait for completion here because yarg's check does
      // not support it yet, the purpose of require-main is really
      // to load modules as quickly as possible, before Electron's
      // ready event. If we want to reliably block running mocha
      // until all require-mains are through, we need to pull this
      // out of check.
      helpers
        .handleRequires(argv['require-main'])
        .catch(err => {
